    ...
    override func execute(with arguments: ArgumentParser.Result) {
        ...
        if let destinationPath = arguments.get(destinationPath) {
            if let sourceRootPaths = arguments.get(sourceRootPaths) {
                ...
                if scanPlugins {
                    PluginizedNeedle.generate(from: sourceRootPaths, excludingFilesEndingWith: excludeSuffixes, excludingFilesWithPaths: excludePaths, with: additionalImports, headerDocPath, to: destinationPath)
                } else {
                    Needle.generate(from: sourceRootPaths, excludingFilesEndingWith: excludeSuffixes, excludingFilesWithPaths: excludePaths, with: additionalImports, headerDocPath, to: destinationPath)
                }
            } else {
