...
class PluginizedDependencyGraphParserTests: AbstractPluginizedParserTests {
    ...
    func test_parse_withTaskCompleteion_verifyTaskSequence() {
        ...
        do {
            _ = try parser.parse(from: [fixturesURL], excludingFilesEndingWith: ["ha", "yay", "blah"], using: executor)
        } catch {
            XCTFail("\(error)")
        }
    ...
    func test_parse_withTaskCompleteion_verifyResults() {
        ...
        do {
            let (components, pluginizedComponents, imports) = try parser.parse(from: [fixturesURL], excludingFilesEndingWith: ["ha", "yay", "blah"], using: executor)
