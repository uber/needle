...
public class __PluginExtensionProviderRegistry {
    ...
    public func registerPluginExtensionProviderFactory(`for` componentName: String, _ pluginExtensionProviderFactory: @escaping (PluginizedComponentProtocol) -> AnyObject) {
    ...
    func pluginExtensionProvider(`for` component: PluginizedComponentProtocol) -> AnyObject {
        // Lock on `providerFactories` access.
        lock.lock()
        defer {
            lock.unlock()
    ...
    private var providerFactories = [String: (PluginizedComponentProtocol) -> AnyObject]()
